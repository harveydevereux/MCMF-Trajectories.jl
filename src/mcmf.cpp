...
int MCMF_CS2::run_cs2(bool debug, int & current_min, int & cost, std::vector<int> & v)
{
...
void MCMF_CS2::solution(std::vector<int> & v)
{
	NODE *i;
	ARC *a;
	long ni;
  ...
  // surely this is unitilialised? therefor an undefined
	...
	for ( i = _nodes; i < _nodes + _n; i ++ ) {
		...
		for ( a = i->suspended(); a != (i+1)->suspended(); a ++) {
			if ( _cap[ N_ARC (a) ]  > 0 ) {
				v.push_back(ni);
				v.push_back(N_NODE(a->head()));
				v.push_back(_cap[ N_ARC(a) ] - a->rez_capacity());
			}
		}
    }
}
